...
Find the domain file(s) whose `systems[]` includes `inventory-system` or `combat-system` (typically `entities-items.yaml`). Load those domain files. For cross-system loot tables spanning multiple domains, load all domain files.
